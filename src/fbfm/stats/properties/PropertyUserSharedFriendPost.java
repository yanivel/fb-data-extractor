...
public class PropertyUserSharedFriendPost extends Stat{
  ...
  protected StatResponse calculateStat(FacebookClient facebookClient, SetMultimap<String,Object> parameters )
  {
    ...
    for (Object profileId : params) {     
        ...
        Set<JsonObject> sharedPostSet;
        // if cache
        if (cache.hasCacheData(userId, "sharedPosts", "Sets")) {
            Map<Object, Object> sharedPostsSets = cache.getCacheData(userId, "sharedPosts", "Sets");
            sharedPostSet = (Set<JsonObject>)sharedPostsSets.get("sharedPostsJsonObjects");
            
            for (JsonObject post : sharedPostSet) {
        ...
        // else no cache yet
            ...
            sharedPostSet = new HashSet<JsonObject>();
